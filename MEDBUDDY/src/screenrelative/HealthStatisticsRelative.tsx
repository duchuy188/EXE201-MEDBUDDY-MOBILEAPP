...
const HealthStatisticsRelative: React.FC = () => {
  ...
  // Thêm validation cho dữ liệu trước khi render chart
  const validBloodPressureData = bloodPressureData.filter(item => 
    item && 
            ...
            <View style={{alignItems: 'center', marginVertical: 8}}>
              <PieChart
                data={weeklyOverview.map(item => ({
                  name: item.name,
                  population: Number(item.value) || 0, // Đảm bảo là số
                  color: item.color,
            ...
            <>
              <View style={styles.sectionCard}>
                ...
                <View style={{alignItems: 'center'}}>
                  <LineChart
                    data={{
                      labels: validBloodPressureData.map((item, index) => 
                        item.day || `Day ${index + 1}`
                      ),
                      datasets: [
                        {
                          data: validBloodPressureData.length > 0 
                            ? validBloodPressureData.map(item => Number(item.systolic) || 0)
                            : [0], // Fallback data
                          color: () => '#ef4444',
                          strokeWidth: 2,
                        },
                        {
                          data: validBloodPressureData.length > 0 
                            ? validBloodPressureData.map(item => Number(item.diastolic) || 0)
                            : [0], // Fallback data
                          color: () => '#3B82F6',
                          strokeWidth: 2,
                        },
